...
function RepItems()
	plsr.Inventory.Items:RegisterUse("rep_voucher", "RandomItems", function(source, item)
		local char = plsr.Fetch:CharacterSource(source)
		if item.MetaData.Reputation and ((item.MetaData.Amount and tonumber(item.MetaData.Amount) or 0) > 0) then
			plsr.Reputation.Modify:Add(source, item.MetaData.Reputation, item.MetaData.Amount)
